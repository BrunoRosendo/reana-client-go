...
func parseLogsFilters(filterInput []string) (utils.Filters, error) {
	...
	filters := utils.NewFilters(singleValueFilters, multiValueFilters)

	err := filters.AddFilters(filterInput)
	if err != nil {
		return filters, err
	}
	...
	computeBackendsSlice := make([]string, 0, len(utils.ReanaComputeBackends))
	for key := range utils.ReanaComputeBackends {
		computeBackendsSlice = append(computeBackendsSlice, key)
	}
	err = filters.ValidateValues("compute_backend", computeBackendsSlice)
	if err != nil {
		return filters, err
	}
